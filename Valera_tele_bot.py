...
def start(message):
    ...
    buttonPere = types.KeyboardButton('Список языков')
    markup.row(button, buttonP, buttonPere)
    bot.send_message(message.from_user.id, f'Приветствую, {message.from_user.first_name}!👋 Меня зовут Валера. Вот функции, которые я выполняю: \n'
                                           ...
                                           f'~ Я могу поговорить с тобой на языках народов России, чтобы выбрать язык нажми на кнопку "Список языков"',
                     reply_markup=markup)
...
def scenari(message):
    ...
    if message.text.lower() == 'викторина':
        bot.send_message(message.from_user.id, 'Итак, викторина. Но это необычная викторина. Представьте обычную настольную игру, в которой бросаешь кубик и ходишь. \n'
                                               ...
                                               '~ Всегда можешь вернуться к спику игр при помощи функции "/start" \n'
    ...
    if message.text.lower() == 'интересные факты' or message.text.lower() == 'повторить интересные факты':
        if pol == 'ж':
            bot.send_message(message.from_user.id,
                             ...
                             '~ Ты можешь досрочно закончить игру написав: "/stop_fact" или "/start" \n'
    ...
    if message.text.lower() == 'город № 20':
        if podskazka_F == 3:
            ...
            podskazka_F += 1
    if message.text.lower() == 'список языков':
        bot.send_message(message.from_user.id, '*Вот языки, на которых говорят народы России, и которые я знаю:*\n'
                                               '*Официальный в РФ*\n'
                                               '~ Русский\n'
                                               '*Башкортастан*\n'
                                               '~ Башкирский\n'
                                               ...
                                               '*Еврейская АО*\n'
                                               '~ Идиш\n'
                                               '   Чтобы я перевел твое слово на любой из этих языков, то напиши вот так:\n'
                                               'Язык, на который надо перевести и слово, которое надо перевести.\n'
                                               '*Все через пробел!*'
                         ,
                         parse_mode='Markdown')
    ...
    if lang.lower() == 'русский':
        ...
        bot.send_message(message.from_user.id, f'Твое слово: {slovo}\n'
                                               f'Перевод: {translated}\n'
                                               f'Перевод на {lang.lower().capitalize()}')
    if lang.lower() == 'башкирский':
        to_translate = slovo
        translated = GoogleTranslator(source='auto', target='ba').translate(to_translate)
        bot.send_message(message.from_user.id, f'Твое слово: {slovo}\n'
                                               f'Перевод: {translated}\n'
                                               f'Перевод на {lang.lower().capitalize()}')
    if lang.lower() == 'татарский':
        to_translate = slovo
        translated = GoogleTranslator(source='auto', target='tt').translate(to_translate)
        bot.send_message(message.from_user.id, f'Твое слово: {slovo}\n'
                                               f'Перевод: {translated}\n'
                                               f'Перевод на {lang.lower().capitalize()}')
    if lang.lower() == 'азербайджанский':
        to_translate = slovo
        translated = GoogleTranslator(source='auto', target='az').translate(to_translate)
        bot.send_message(message.from_user.id, f'Твое слово: {slovo}\n'
                                               f'Перевод: {translated}\n'
                                               f'Перевод на {lang.lower().capitalize()}')
    if lang.lower() == 'казахский':
        to_translate = slovo
        translated = GoogleTranslator(source='auto', target='kk').translate(to_translate)
        bot.send_message(message.from_user.id, f'Твое слово: {slovo}\n'
                                               f'Перевод: {translated}\n'
                                               f'Перевод на {lang.lower().capitalize()}')
    if lang.lower() == 'финский':
        to_translate = slovo
        translated = GoogleTranslator(source='auto', target='fi').translate(to_translate)
        bot.send_message(message.from_user.id, f'Твое слово: {slovo}\n'
                                               f'Перевод: {translated}\n'
                                               f'Перевод на {lang.lower().capitalize()}')
    if lang.lower() == 'идиш':
        to_translate = slovo
        translated = GoogleTranslator(source='auto', target='yi').translate(to_translate)
        bot.send_message(message.from_user.id, f'Твое слово: {slovo}\n'
                                               f'Перевод: {translated}\n'
                                               f'Перевод на {lang.lower().capitalize()}')
